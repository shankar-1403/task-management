...
export function MyTasksSectionBlock({
  ...
  return (
    <section className="my-tasks-section">
      <header className="my-tasks-section-header">
        <button
          type="button"
          className="my-tasks-section-toggle"
        ...
        >
          {collapsed ? (
            <IconChevronRight
              size={ICON_SIZE.sm}
              stroke={ICON_STROKE}
              className="app-icon app-icon--sm my-tasks-section-chevron"
              aria-hidden
            />
          ) : (
            <IconChevronDown
          ...
          <h2 className="my-tasks-section-title">{section.name}</h2>
          {isFixedMyTasksSection(section.id) && (
            <span className="my-tasks-section-pill">Project column</span>
          )}
          <span className="my-tasks-section-count">{openCount}</span>
        </button>
        {!isFixedMyTasksSection(section.id) && onDeleteSection && (
          <button
            type="button"
